...
"""Implementation of an compute service that uses Nova as the backend."""
...
def _novaclient_from_endpoint(context, endpoint, version=2):
    ...
    if endpoint.startswith('https://'):
        ...
        params['insecure'] = CONF.nova.nova_api_insecure
        params['ssl_compression'] = False
        if CONF.nova.nova_cafile:
            params['cacert'] = CONF.nova.nova_cafile
    return novaclient.Client(str(version), endpoint, **params)


def get_api_servers():
    """Shuffle a list of CONF.nova.nova_api_servers and return an
    iterator that will cycle through the list, looping around to the
    beginning if necessary.
    """
    api_servers = []

    for api_server in CONF.nova.nova_api_servers:
...
class NovaClientWrapper(object):
    ...
    def _create_static_client(self, context, endpoint, version):
        ...
        return _novaclient_from_endpoint(context, endpoint, version)
    ...
    def call(self, context, version, method, *args, **kwargs):
        ...
        for attempt in range(1, num_attempts + 1):
            ...
            try:
                ...
                LOG.exception("Error contacting nova server "
...
class NovaComputeServiceV2(object):
    """Provides storage and retrieval of instnace objects within Nova."""

    def __init__(self, client=None):
        self._client = client or NovaClientWrapper()

    def show(self, context, instance_uuid):
        """Returns a dict with insntace data for the given instance uuid.

        :param context: The context object to pass to compute client
        :param instance_uuid: The UUID of the instance
        """
        try:
            instance = self._client.call(context, 2, 'get', instance_uuid)
        except Exception:
            _reraise_translated_compute_exception(instance_uuid)

        instance = _translate_from_nova(instance)

        return instance


def _translate_from_nova(instance, include_locations=False):
...
# NOTE(bcwaldon): used to store non-string data in nova metadata
...
def _reraise_translated_compute_exception(instance_uuid):
    """Transform the exception for the instance but keep its
    traceback intact."""
    exc_type, exc_value, exc_trace = sys.exc_info()
    new_exc = _translate_compute_exception(instance_uuid, exc_value)
    six.reraise(new_exc, None, exc_trace)


def _translate_compute_exception(instance_uuid, exc_value):
    if isinstance(exc_value, (novaclient.exc.Forbidden,
                              novaclient.exc.Unauthorized)):
        return exception.InstanceNotAuthorized(instance_uuid=instance_uuid)
    if isinstance(exc_value, novaclient.exc.NotFound):
        return exception.InstanceNotFound(instance_uuid=instance_uuid)
    if isinstance(exc_value, novaclient.exc.BadRequest):
        return exception.InstanceRequest(instance_uuid=instance_uuid,
                                         response=six.text_type(exc_value))
    return exc_value


def get_compute_service(context):
    """Create an nova_compute service."""
    return NovaComputeServiceV2()
